...
{
    public static async Task<bool> InsertDictionaryAsync(this MySqlDbContext db, UserBase user, string name, CancellationToken cancellationToken = default)
    {
        ...
        await db.Dictionaries.AddAsync(new Types.Common.Dictionary(Guid.NewGuid(), name, user.Guid), cancellationToken);
